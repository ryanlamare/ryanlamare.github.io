...
//
//   node teaching/ler565/headcount/relay/dev-relay.js [port]     (default 8787)
//
// Run it beside ./serve.sh and two devices on the same network can play a real
// game: laptop on http://<lan-ip>:8000/teaching/ler565/headcount/, phone on the
  ...
  return new Promise((resolve) => {
    server.listen(port, '0.0.0.0', () => {
      if (!quiet) {
        ...
        console.log(
          `  Run ./serve.sh too, then open the game${ip ? ` at http://${ip}:8000/teaching/ler565/headcount/` : ''}`
        );
