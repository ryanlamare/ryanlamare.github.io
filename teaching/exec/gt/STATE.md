...
- **Wording still to veto** (all Claude's): the chicken build of 13 Sep (the slide 10 bullet "You have thirty seconds a round. Do nothing, and you are going straight"; the phone's round-four note "At any moment before you choose, you can throw your steering wheel out of the window. If you do, you cannot swerve, and Sam is told the moment it happens"; the button "Hold to throw the wheel out"; the card wording "froze, and went straight"; the board tiles "Froze at the wheel · of drivers let the clock run out, and went straight" and "· of drivers, with a wheel to throw"; the award lines "The fastest to pick, on average, in every game on a clock" and "The most deliberate, on average, in every game on a clock"); the auction slides, the reputation slide's two chips, the brinkmanship figure, the Sun Tzu copy and its toy numbers, the cautionary slide's bullets after the opening question, the penalty widget's bullets, the airtime slide (facts unchecked against the 550 source), the Rebel bullets in m5, and the m6 rules bullet that now reads "the odds in the table".
...
- **Chicken on a clock** (13 Sep, Ryan's idea, built the same day, awaiting his drive): every round is a thirty-second approach on the phone, the deck's own two cars closing over the clock; swerve or straight is one tap that commits; a driver who has not chosen at zero goes straight (their own phone sends it, with the full clock as its time, so the rules read it as frozen). From round four the wheel is thrown *during* the approach by press-and-hold, and the other driver is told at once ("Alex threw the wheel out and is locked in to going straight"). The reveal plays the round back: your car, their car, the crash or the pass, then the money. Wire gained a sixth field (seconds taken) and lost the keep-the-wheel line; chicken.js, scores.js and the deck agree. The board has two Froze at the wheel tiles; m8 has Quick draw and Careful marksman (shortest and longest mean decision time over every game on a clock, three timed decisions to qualify, nobody named on the frozen count). A round starts only when both drivers have pressed to proceed (a ready line per driver per round, ignored by the rules), so the two clocks start together; rounds four to six put the wheel rule on that screen, so the road is the same layout every round. The Worker's room cap is 400 lines and a pair's game is now about 30, so twelve pairs is the ceiling in one room; ten is comfortable. `?clock=5` on the game page shortens the clock for a rehearsal. **The direction for every game** (Ryan, 13 Sep): a tangible decision, an outcome you watch, the money after; split or steal (the two golden balls opening) is next, then rock paper scissors and the inspection game in m6, one game at a time.
