...
(function(){
  ...
  const CELLW=94, PITCH=103, X0=150, CELLH=96, TIERH=110, COLS=10;
  const floorY=k=>G-90-k*TIERH;
  ...
  window.M4_BLOCK=function(svg,opts){
    ...
    let T=3, layers={}, cells=[], pairs=new Map(), queue=[], lastStart=0, list=[], onClickFn=null, loop=null, tweens=[];
    ...
    function buildScene(){
      ...
      for(let k=0;k<T;k++){const fy=floorY(k);mk('path',{d:'M66 '+fy+' H132',stroke:INK,'stroke-width':1.5});
        for(let s=0;s<6;s++){const y=fy+18*s+18;if(y<G)mk('path',{d:'M'+(74+s*8)+' '+y+' h14',stroke:INK,'stroke-width':1.5,opacity:.6});}}
      ...
      for(let k=0;k<T;k++){
        ...
        for(let c=0;c<COLS;c++){
          ...
          mk('path',{d:'M'+x0+' '+(ty+30)+' h'+CELLW+' M'+x0+' '+(ty+60)+' h'+CELLW+' M'+(x0+30)+' '+ty+' v30 M'+(x0+64)+' '+(ty+30)+' v30 M'+(x0+40)+' '+(ty+60)+' v30',stroke:MORTAR,'stroke-width':1.2},g);
          mk('rect',{x:x0,y:fy-8,width:CELLW,height:8,fill:FLOORC},g);
          const win=mk('rect',{x:x0+68,y:ty+10,width:18,height:13,fill:BAR,opacity:.28},g);
          mk('path',{d:'M'+(x0+74)+' '+(ty+10)+' v13 M'+(x0+80)+' '+(ty+10)+' v13',stroke:DARK,'stroke-width':1.5},g);
          const lamp=mk('rect',{x:x0+68,y:ty+10,width:18,height:13,fill:OCHRE,opacity:0},g);
          const num=mk('text',{x:x0+8,y:ty+20,'font-family':'Jost,sans-serif','font-weight':'700','font-size':'12','letter-spacing':'1','fill':BAR,opacity:.85},g);
      ...
      /* the outer wall, its gate, and the watchtower */
      ...
      /* the watchtower, over the stairwell */
      ...
      cells.forEach(c=>{
        const d=mk('g',{class:'door','data-k':c.i+1},layers.doors);
        c.bars=[];for(let b=0;b<5;b++)c.bars.push(mk('path',{d:'M'+(c.x0+6+b*20.5)+' '+(c.ty)+' V'+c.fy,stroke:BAR,'stroke-width':3.5,'stroke-linecap':'round'},d));
        mk('path',{d:'M'+(c.x0+6)+' '+(c.ty+12)+' H'+(c.x0+88)+' M'+(c.x0+6)+' '+(c.fy-12)+' H'+(c.x0+88),stroke:BAR,'stroke-width':3.5,'stroke-linecap':'round'},d);
        mk('circle',{cx:c.x0+84,cy:c.ty+CELLH/2,r:4,fill:BAR},d);
    ...
    /* ================= the prisoners ================= */
    function makePair(){
      const g=mk('g',{class:'pair'},layers.outside);
      const a=mk('g',{transform:'translate(-32,-46) scale(.36)'},g);S.prisonerNodes(a);
      const b=mk('g',{transform:'translate(4,-46) scale(.36)'},g);S.prisonerNodes(b);
      mk('path',{d:'M-9 -24 q5 4 10 0',stroke:INK,'stroke-width':2,fill:'none','stroke-dasharray':'2 2'},g);
    ...
    function setList(L){
      list=L;
      const need=Math.min(5,Math.max(3,Math.ceil(L.length/COLS)));
      ...
      if(need!==T||!cells.length||gone){T=need;const had=gone?new Map():new Map(pairs);pairs=new Map();queue=[];buildScene();
    ...
    async function breakout(k,route){
      ...
      if(route==='m'){ /* out by agreement, and marched back in */
        ...
        await walk(p,[[c.cx-70,c.fy]],160);await wait(500);await walk(p,[[c.cx,c.fy]],160);
        enter(p,c);await tween(300,f=>setDoor(c,1-f));return;
      }
      c.state='out';
      if(route==='r'){ /* down through the floor, up outside the wall, and away */
        await tween(600,f=>{c.hole.setAttribute('rx',(30*f).toFixed(1));c.hole.setAttribute('ry',(5*f).toFixed(1));});
    ...
    function restore(k,route){const c=cells[k-1], g=list[k-1];if(!c||!g)return;const p=pairs.get(g.v);
      ...
      if(route==='r'){c.hole.setAttribute('rx','30');c.hole.setAttribute('ry','5');}
