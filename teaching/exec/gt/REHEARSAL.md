...
- **The m4 dilemmas** (the closing exercise, two touches): one phone per Price Wars team, so open **ryanlamare.com/teaching/exec/gt/m4/pds/** in two student windows only, fill the three cards, lock each in, and watch the matrix cards land on The room's dilemmas; walk them with the arrow key. Later, on How would you get out of yours?, press **Open the question on the phones** and answer it from the same two windows; the chips land on the cards, then walk again. `?demo=1` on the m4 deck URL shows six demo dilemmas without a room. The Axelrod ranking is a /go poll: rank the strategies from three windows (`node teaching/exec/gt/trial/m4.js rank 5` posts five more), watch the count, then on the next slide the rows sit in the room's order; press **Run the room's tournament**, and the arrow adds the verdict. Reset `m4-axelrod` from Poll Desk afterwards.
