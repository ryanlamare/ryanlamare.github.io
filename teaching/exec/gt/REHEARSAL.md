...
- **The m4 dilemmas** (the closing exercise, two touches, one phone each since 18 Sep): open **ryanlamare.com/teaching/exec/gt/m4/pds/** in the student windows, give a name, fill the three cards, lock each in, and watch the pairs step off the bus into their cells on The room's dilemmas; click a cell to open it (WHOSE CELL IS THIS? chalks the name), click it or press Esc to close; the arrow keys move on. Later, on How would you get out of yours?, press **Open the question on the phones** and answer it from the same windows; nothing moves until you click a cell, which plays its escape large, and closing it (or clicking the next) breaks the pair out of the block; **Who is still inside?** dims the emptied cells and shows the tally. `node teaching/exec/gt/trial/m4.js --room trial pds 20` fills a rehearsal room (open the deck with `?room=trial`), and `routes` answers the way out once the question is open. `?demo=1` on the m4 deck URL shows fourteen demo dilemmas without a room. The Axelrod ranking is a /go poll: rank the strategies from three windows (`node teaching/exec/gt/trial/m4.js rank 5` posts five more), watch the count, then on the next slide the rows sit in the room's order; press **Run the room's tournament**, and the arrow adds the verdict. Reset `m4-axelrod` from Poll Desk afterwards.
- **Whole-room games** (m1 number game and offers; m5 investment rounds; m7 auction; m8 car market): join from all five windows and your phone *before* pressing the key that opens the round on the deck. m8's market keys are 1, 2, 3 to open rounds, C to close, W for the warranty rule.
