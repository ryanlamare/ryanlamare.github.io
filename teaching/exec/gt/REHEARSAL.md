...
- **The m4 dilemmas** (the closing exercise, two touches, one phone each since 18 Sep): open **ryanlamare.com/teaching/exec/gt/m4/pds/** in the student windows, give a name, fill the three cards (the cell at the top builds as you type), lock each in, and watch the cells fill on The room's dilemmas: the two prisoners walk in and the door slams; walk them with the arrow key (WHOSE CELL IS THIS? chalks the name). Later, on How would you get out of yours?, press **Open the question on the phones** and answer it from the same windows; each cell plays its escape as the answer lands, the walk replays them large, and the last press leaves only the cells still shut lit. `node teaching/exec/gt/trial/m4.js --room trial pds 20` fills a rehearsal room (open the deck with `?room=trial`), and `routes` answers the way out once the question is open. `?demo=1` on the m4 deck URL shows fourteen demo dilemmas without a room. The Axelrod ranking is a /go poll: rank the strategies from three windows (`node teaching/exec/gt/trial/m4.js rank 5` posts five more), watch the count, then on the next slide the rows sit in the room's order; press **Run the room's tournament**, and the arrow adds the verdict. Reset `m4-axelrod` from Poll Desk afterwards.
