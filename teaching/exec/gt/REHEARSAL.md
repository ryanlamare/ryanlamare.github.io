...
- **m2.** *Should you smoke?* is a poll; its count comes back as a line on the two-player tree. The three games share one phone page, but from 20 Sep each rules slide carries its own QR that opens that game directly (`?g=cards`, `?g=ult`, `?g=cent`), as m1 does; nobody passes through Pick your game. The two results slides carry no QR. Pairing is by exact name, which is the fiddly part; watch how it goes. `node trial/m2.js ultimatum 12` (or `centipede`, `cards`) fills a results slide with bot pairs; m2 has no rehearsal room, so they land in the real rooms and the Poll Desk reset comes before any real session. The closer is pen and paper: nothing to rehearse but whether the blank tree on the slide is enough to start from.
